...
- 問い合わせ一覧のキーワード検索・カテゴリ/状態フィルタ: キーワード検索でヒット件数が正しく絞り込まれること、カテゴリ/状態フィルタが単独・組み合わせで機能すること、「クリア」で元の一覧に戻ることを確認済み
...
- **会話検索の日付範囲フィルタ**: キーワード検索・カテゴリ/状態フィルタは`ConversationList`で対応済みだが、日付範囲での絞り込みは未実装
